...
test('update checker falls back to GitHub releases for non-npm plugins', async () => {
  ...
  await mkdir(moduleDir, { recursive: true })
  await atomicWriteJson(join(dir, 'package.json'), { name: 'profile', dependencies: { 'Vibe-Skills': 'github:owner/Vibe-Skills' } })
  ...
  assert.equal(entry.status, 'available'); assert.equal(entry.latestVersion, '2.3.0'); assert.equal(entry.source, 'github')
...
const p = args.indexOf('--profile'); const dir = path.join(home, 'profiles', args[p + 1])
