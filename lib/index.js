  ...
  return {
    async dispatch(method, payload, query) {
      switch (method) {
        ...
        case 'updates.check': return health.checkUpdates(requireString(payload, 'profile'), Array.isArray(payload.packageNames) ? payload.packageNames.filter((name) => typeof name === 'string') : undefined)
