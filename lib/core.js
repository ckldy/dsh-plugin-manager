...
export class PluginHealthService {
  ...
  async checkGithubUpdate(plugin) {
    ...
    const manifestVersion = typeof plugin.manifest?.version === 'string' && plugin.manifest.version.trim() ? plugin.manifest.version.trim() : plugin.version
    const comparison = compareVersions(latestVersion, manifestVersion)
    // A manifest without a parseable version (e.g. a github: spec as the
    // version) cannot be compared, but an update to the latest tag is still
    // offered rather than silently reporting "unknown".
    ...
    const result = { packageName: plugin.packageName, currentVersion: manifestVersion, latestVersion, ref: tag, registryUrl: `https://github.com/${repo.owner}/${repo.repo}/releases`, status: updateAvailable ? 'available' : 'current', updateAvailable, source: 'github', repositoryUrl: repo.repositoryUrl, installSpec }
...
export class MutationManager {
    ...
    const items = resolvedItems.map(({ item: rawItem, manifest }) => {
      ...
      if (analysis.duplicate) {
        ...
        if (spec && previous.requested !== spec) {
          analysis.replacing = { packageName: previous.packageName, previousRequested: previous.requested, installSpec: spec }
          analysis.duplicate = undefined
        }
      }
      if (!analysis.integration.eligible) throw new PluginManagerError('not-dsh-plugin', `项目 ${manifest.name || item.name || 'unknown'} 未声明 dsh.bundle.patch，无法自动集成到 DSH`, 422)
  ...
  async restore(snapshot, dir) {
    ...
    await run('pnpm', ['install', '--offline', '--frozen-lockfile'], { cwd: dir, timeout: 10 * 60_000 })
  }
  async runTask(task, plan) {
    ...
    try {
      ...
      if (args.length === 4) { this.log(task, '没有需要执行的变更'); task.phase = 'validate' }
        ...
        const result = await run(this.dshCommand, args, { cwd: dir, timeout: 15 * 60_000, onLog: (_kind, text) => text.split(/\r?\n/).filter(Boolean).forEach((line) => this.log(task, line)) })
      ...
      task.phase = 'reconcile'; task.progress = 65
        if (plan.kind === 'install') {
          ...
          const bundles = [...new Set([...(installedManifest.dsh?.profile?.bundles ?? []), ...plan.items.filter((x) => !x.analysis.duplicate).map((x) => x.manifest.name)])]
          ...
          this.log(task, `已将 ${bundles.length} 个 DSH Bundle 写入 Profile 启用列表`)
        }
      ...
      if (plan.kind === 'install') for (const entry of plan.items.filter((x) => !x.analysis.duplicate)) {
        ...
        if (!after.bundles.includes(entry.manifest.name)) throw new PluginManagerError('integration-failed', `插件 ${entry.manifest.name} 已安装但未启用到 dsh.profile.bundles`, 500)
      }
      ...
      const validation = await run(this.dshCommand, ['--profile', plan.profile, '--dump-config'], { cwd: dir, timeout: 30_000, onLog: (_kind, text) => text.split(/\r?\n/).filter(Boolean).slice(-20).forEach((line) => this.log(task, line)) })
