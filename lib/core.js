...
export class CatalogService {
  ...
  async githubSearch(query = '', sort = 'stars', page = 1) {
    ...
    return this.cached(`github:${q}:${sort}:${page}`, 5 * 60_000, async () => {
      const url = new URL('https://api.github.com/search/repositories'); url.searchParams.set('q', q); url.searchParams.set('sort', sort === 'updated' ? 'updated' : 'stars'); url.searchParams.set('order', 'desc'); url.searchParams.set('per_page', '30'); url.searchParams.set('page', String(page))
      ...
      const data = await response.json()
      return { items: data.items.map((repo) => normalizeCatalogItem(repo)), total: data.total_count }
    })
  }
  ...
  mergeCatalogItems(items) {
    ...
    return [...byRepository.values()].map((item) => ({ ...item, iconUrl: item.iconUrl ?? repositoryAvatarUrl(item.repositoryUrl) }))
  }
  async list(query = '', sort = 'stars') {
    ...
    unique.sort(sort === 'updated' ? (a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt)) : (a, b) => b.stars - a.stars)
