...
export class PluginHealthService {
  ...
  async checkUpdate(plugin) {
    ...
    try {
      ...
      const result = { packageName: plugin.packageName, currentVersion: plugin.version, latestVersion, registryUrl: `https://www.npmjs.com/package/${encodeURIComponent(plugin.packageName)}`, status: comparison === null ? 'unknown' : comparison > 0 ? 'available' : 'current', updateAvailable: comparison === null ? false : comparison > 0 }
