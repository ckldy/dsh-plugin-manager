...
export function analyzeInstall(current, candidate) {
  ...
  if (!integration.eligible) risks.unshift({ level: 'info', kind: 'generic-install', label: '通用安装模式', detail: '项目未声明 dsh.bundle.patch；将作为 Profile 普通依赖安装，不会自动写入 dsh.profile.bundles。请按项目 README 中的启动或配置说明完成 DSH 集成。' })
  return { duplicate, reused, additions, versionConflicts, overlaps, capabilities: incomingCaps, scripts, nativeDependencies, risks, integration, installMode: integration.eligible ? 'dsh-bundle' : 'profile-package' }
  }
...
export class MutationManager {
  ...
  async planInstall(profile, resolvedItems) {
    ...
    const items = resolvedItems.map(({ item: rawItem, manifest }) => {
      ...
      if (!manifest.name || typeof manifest.name !== 'string') throw new PluginManagerError('invalid-manifest', '项目 package.json 必须声明 name，才能生成安全的安装方案', 422)
      if (!analysis.integration.eligible && analysis.replacing) {
        // An existing external package may be wired by the Profile's own patch.
        // Keep that wiring unchanged when the package is updated.
        analysis.replacing.preserveIntegration = true
        analysis.risks.push({ level: 'warning', kind: 'preserved-integration', label: '保留现有 Profile 集成', detail: '新版本未声明 dsh.bundle.patch；更新不会新增 Bundle 注册。' })
      }
    ...
    const actionableItems = items.filter((entry) => !entry.analysis.duplicate)
    const noChanges = items.length > 0 && actionableItems.length === 0
    const plan = { id: randomUUID(), kind: 'install', profile, createdAt: Date.now(), baselineHash: stableHash({ manifest: current.manifest, bundles: current.bundles }), items, summary: { reuseDependencies: items.reduce((n, x) => n + x.analysis.reused.length, 0), addDependencies: items.reduce((n, x) => n + x.analysis.additions.length, 0), versionConflicts: items.flatMap((x) => x.analysis.versionConflicts), hardConflicts, blockedUpdates, duplicatePlugins, noChanges, installModes: items.map((x) => ({ packageName: x.manifest.name, mode: x.analysis.installMode, command: `dsh plugin --profile ${profile} add ${x.item.installSpec}`, integration: x.analysis.integration.eligible ? '自动注册到 dsh.profile.bundles' : '作为普通 Profile 依赖安装；参照 README 完成后续集成' })), updates: items.filter((x) => x.analysis.replacing).map((x) => ({ packageName: x.analysis.replacing.packageName, from: x.analysis.replacing.previousRequested, to: x.analysis.replacing.installSpec, mode: x.analysis.replacing.mode ?? 'replace' })), requiresRestart: true } }
  ...
  async execute(planId, hash, allowConflicts = false) {
    ...
    if (plan.kind === 'install' && plan.summary.noChanges) throw new PluginManagerError('already-current', '所选项目已按相同版本和来源安装，无需重复安装', 409, plan.summary.duplicatePlugins)
    ...
    try {
      ...
      task.phase = 'reconcile'; task.progress = 65
        ...
        const bundles = plan.kind === 'install'
          ? [...new Set([...currentBundles, ...plan.items.filter((x) => !x.analysis.duplicate && x.analysis.integration.eligible && !x.analysis.replacing?.preserveIntegration).map((x) => x.manifest.name)])]
        ...
        this.log(task, plan.kind === 'install'
          ? `已将 ${bundles.length} 个 DSH Bundle 写入 Profile 启用列表；普通项目仅作为依赖保留`
      ...
      if (plan.kind === 'install') for (const entry of plan.items.filter((x) => !x.analysis.duplicate)) {
        ...
        if (entry.analysis.integration.eligible && !entry.analysis.replacing?.preserveIntegration && !after.bundles.includes(entry.manifest.name)) throw new PluginManagerError('integration-failed', `插件 ${entry.manifest.name} 已安装但未启用到 dsh.profile.bundles`, 500)
      }
